...
\begin{table}[H]
	...
	\begin{tabular}{ccccccccc}
		\toprule
		\textbf{Exercise} & \textbf{Subjects} & \textbf{Sessions}	& \textbf{Trials} & \textbf{Reps} & \textbf{Samples} & \textbf{10-fold} & \textbf{LOSO} & \textbf{LOTO} \\
		\midrule
...
\end{table}
\subsubsection{Set\_A: Statistical Features}
...
\subsubsection{Set\_B: Histogram bins Features}
...
\subsubsection{Set\_C: Self-Similar Features}
...
\subsubsection{Set\_D: Physical Features}
...
\subsubsection{Set\_E: Orientation Independent Features}
...
In this section we compare the performance of four different classifiers on all five featuresets. Table \ref{accuracy_classifier_featureset} and \ref{f1_classifier_featureset} show the results respectively in Accuracy and F-score. We used 10-fold cross validation to measure the performance. A total of 150,000 data points were included in the validation set. Table \ref{dataset_statistics} shows total number of samples for each activity separately. It can be seen how classifiers performance varies over featuresets. All classifiers on \textit{set\_B} and \textit{set\_A} deliver their best performance. On the other hand,  the performance achieved on a given featureset has been affected by changing classifiers. FNN and SVM gained the best results comparing with two other classifiers. Also, it seems clear that the \textit{set\_E} (Orientation Invariant features) provide insufficient information for successfully tackling activity recognition, since the accuracy never exceeds 62\%, whereas other featuresets achieve accuracies over 75\%. Interestingly, the \textit{set\_B} (Histogram Features) with having the least complexity in computation gained the best performance among other featuresets. In fact, accuracy achieved by the top-performing models (FNN and SVM) are significantly better when they use histogram features. This could happen due to choosing best bin width based on length of activity and windows size.
...
\end{table}
\subsubsection{RQ1: Which classifier performs better on gym exercise recognition?}

\subsubsection{RQ1: How are different evaluation methods?}
...
\subsection{FNN training speed over different feature sets }
...
\end{figure} 
\subsection{Impact of Imbalanced dataset}
\begin{figure}[H]
	\centering
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{Definitions/images/fnn_balanced.png}
		\caption{Balanced dataset}
		\label{fig:fnn_balanced}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{Definitions/images/fnn_imbalanced.png}
		\caption{Imbalanced}
		\label{fig:fnn_imbalanced}
	\end{subfigure}%
	\caption{Comparing impact of undersampling on FNN}
	\label{fig:imbalance_dataset}
\end{figure} 
