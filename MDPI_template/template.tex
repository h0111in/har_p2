...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
With the rise of life expectancy and ageing of population, the development of new technologies that focuses on elderly healthcare has become a challenge \cite{hong2008activity}. Human Activity Recognition (HAR) using Wearables is one of the most promising assistive technologies to support older people's daily life \cite{wang2019survey}. Fall risk assessment of elderly patients \cite{sow2013mining}, physical fitness monitoring\cite{morris2014recofit}, medical diagnosis \cite{gonzalez2015features} are to name a few. Simultaneously, Wearables are increasingly pervasive. They are small in size, relatively cheap and ubiquitously used, which has enabled enormous potential in human-centred applications. Therefore, implementation a HAR system to use the device's resources and deliver the most accurate result in the form of recognizing a certain activity or counting it \cite{schilit1994context} is vital.\\
An HAR system composed of essentially two major parts: (1) A data acquisition unit responsible for capturing human action, (2) A processing unit to recognize certain activity among a stream of data\cite{rosati2018comparison}. In the first component, human activity is captured by different type of sensors (e.g., accelerometer, microphone, camera). Then, within the second component, there is a pattern recognition - HAR model that classifies certain activities from input signal. This HAR model typically consists of three phases. First, there is a processing operation to \textit{extracts features} from raw signal. Second, in \textit{classification }phase a classifier is trained on extracted features. Then, in the third phase, there is an \textit{evaluation} method to ensure that the classification provides required performance\cite{kolodziej2019registration}.  Figure \ref{fig:main_approach} shows a start-to-finish pipeline of HAR.\\
...
Considering that an exercise activity is inherently more repetitive rather than a non-exercise activity, having a featureset that can capture the repetitive behaviour of signal is helpful. Morris et. al. presented a featureset designed based on the idea of extracting repetitions forms of signal \cite{morris2014recofit}. These featursets can be extracted by calculating the convolution of a signal with a shifted version of itself (autocorrelation) or by extracting the components of the signal in the frequency domain. We extracted a number of self-similar features from our data, as shown in~Table~\ref{features_table}, features with code SS1-SS11 \emad{In the table, the SS features do not make sense.}. SS6 composed of 10 power bins \emad{what are power bins?}.
...
\begin{table}[H]
	\caption{Accuracy for each classifier and for all each feature-set using 10-fold cross validation \emad{we should only highlight the best performing featureset,i.e., one per column.}}
	\centering
...
\emad{we should create distributions here based on the different activities, and see if we find a statistically signficant difference between the different classifiers.} 

\emad{also, the results in Table 4 are the average of all the experiments or what activity are these numbers based on.}

A expected and confirmed by our results in RQ1, different classifiers perform differently. Hence, one question that we aim to answer is whether certain classifiers perform better than others. Therefore, in this research question we do an empirically compare the performance of different classifiers in HAR. We use four popular classifiers in HAR including SVM, KNN, FNN, and DT. It is important to mention that we evaluate the model using default configurations. We are not looking for optimal setup of each classifier \emad{so which configuration do we use? Default?}. 
...
\emad{Are Tables 5 and 6 ever referenced to in the text?}
