...
Table \ref{f1_classifier_featureset} shows the performance for each featureset (columns 2-6) for the various classifiers (rows 2-5). We highlight the best performance for each featureset in the Table. It can be seen that the best performing featuresets are \textit{statistical featureset} (ST\_Set) and \textit{Histogram bins} (HB\_Set), achieving F1 of approximately 95\%. The remaining featuresets that perform well are the \textit{self-similar featureset} achieving a maximum F1 of 89.18\%, the \textit{physical featureset} achieving a maximum F1 of 85.34\% and \textit{Orientation independent featureset} achieving a maximum F1 of 78.47\%.
